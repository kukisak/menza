...
{
    private static final long serialVersionUID = -8106624677419348436L;
                ...
                throws IOException 
    {
        ...
        */    	
    	 ...
    	 */
        resp.sendRedirect("/index.jsp?site=reservationOrder");
    }
    ...
    private void saveReservation(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    	...
    	try {    		
	    	...
	    	/**
	    	 * 	TODO proc ta podminka na data nejde rpimo v tom query???
	    	 */
    	    ...
    	    try {
    	        ...
    	        if (results.iterator().hasNext()) {    	        	    	        	
    	            for (Reservation resOrder : results) {    	            	
    	            	//if((resOrder.getDateStart().before(dateStart)) &&(resOrder.getDateEnd().after(dateEnd)))
    	        ...
    	        if ( countEngaged > Max_count ) {
    	        	resp.sendRedirect("/index.jsp?site=reservationOrder&problem=noSpace");
    	        } else if(count > Max_count) {
    	        	resp.sendRedirect("/index.jsp?site=reservationOrder&problem=noSpace");
    	        } else if(dateStart.before(dateEnd)) {
    	        	resp.sendRedirect("/index.jsp?site=reservationOrder&problem=revDate");
    		...
    		//resp.sendRedirect("/reservationOrder.jsp");
    	} finally {
            pm.close();
        }
    }
    ...
    private void editReservation(HttpServletRequest req, HttpServletResponse resp) throws IOException {
    	try {    		    		
    		resp.sendRedirect("/index.jsp?site=reservationOrder&reservationID="+req.getParameter("reservationID")+"");
    	} catch (Exception e) {
    		//resp.sendRedirect("/reservationOrder.jsp");
    	}
    }
